...
class IikoIncomingReceiptsPostgresTests(unittest.TestCase):
    ...
    def seed(self):
        ...
        with self.engine.begin() as c:
            ...
            c.execute(text("INSERT INTO supply_supplier_documents (id, tenant_id, supplier_order_id, supplier_id, document_type, document_number, document_date, status, supplier_display_name_snapshot, currency, total_amount, financial_role, recorded_by_user_id, recorded_at, created_by_user_id) VALUES (:id, 'receipt-test', :order, :supplier, 'DELIVERY_NOTE', 'DN-RECEIPT-PG', CURRENT_DATE, 'RECORDED', 'МЕГАПРОД ООО', 'RUB', 100, 'SUPPORTING', 95601, now(), 95601)"), {"id": ids["document"], "order": ids["order"], "supplier": ids["supplier"]})
            c.execute(text("INSERT INTO supply_supplier_document_lines (id, tenant_id, supplier_document_id, supplier_order_id, supplier_order_line_id, product_name_snapshot, pricing_basis, package_unit_id_snapshot, unit_name_snapshot, quantity_base, unit_price, line_amount, currency) VALUES (:id, 'receipt-test', :document, :order, :order_line, 'Receipt product', 'UNIT', :unit, 'кг', 2, 50, 100, 'RUB')"), {"id": ids["document_line"], "document": ids["document"], "order": ids["order"], "order_line": ids["order_line"], "unit": ids["unit"]})
    ...
    def test_cycle_constraints_immutability_and_concurrent_lifecycle(self):
        ...
        command.downgrade(self.config, "20260917_0055")
        command.upgrade(self.config, "20260917_0056")
        self.assertEqual(self.revision(), "20260917_0056")
        ids = self.seed()
