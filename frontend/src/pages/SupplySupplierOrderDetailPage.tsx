...
export default function SupplySupplierOrderDetailPage() {
  ...
  return <section className="request-page supply-admin-page purchase-request-page"><div className="request-panel">
    ...
    {order.latest_confirmation && <div className="purchase-request-header"><div><span className="field-label">Ответ поставщика</span><strong>{order.latest_confirmation.response_type === 'CONFIRMED' ? 'Подтверждено' : order.latest_confirmation.response_type === 'PARTIALLY_CONFIRMED' ? 'Подтверждено частично' : 'Отклонено'}</strong></div><div><span className="field-label">Отклонения</span><strong>{order.latest_confirmation.deviation_count}</strong></div><div><span className="field-label">Требуют решения</span><strong>{order.open_required_deviations_count ?? 0}</strong></div><div><span className="field-label">Статус проверки</span><strong>{order.supplier_confirmation_review_state === 'REQUIRES_DECISION' ? 'Требует решения' : order.supplier_confirmation_review_state === 'RESOLVED' ? 'Решения приняты' : 'Без обязательных решений'}</strong></div></div>}
