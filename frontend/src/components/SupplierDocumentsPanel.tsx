  ...
  return <section className="supplier-message-panel supplier-documents-panel">
    ...
    {draft && <div className="supplier-document-editor">
      ...
      <div className="supplier-table-wrap"><table className="supplier-table supplier-document-lines"><thead><tr><th>Позиция</th><th>Расчёт</th><th>Количество</th><th>Цена</th><th>Сумма</th><th></th></tr></thead><tbody>{draft.lines.map((line) => <tr key={line.id}>
        ...
        <td>{line.pricing_basis === 'FIXED_AMOUNT' ? '—' : <span className="input-with-suffix"><input aria-label={`${line.pricing_basis === 'PACKAGE' ? 'Упаковок' : 'Количество'} ${line.product_name_snapshot}`} type="number" min={line.pricing_basis === 'PACKAGE' ? '1' : '0.000001'} step={line.pricing_basis === 'PACKAGE' ? '1' : '0.000001'} value={line.pricing_basis === 'PACKAGE' ? line.packages_count ?? '' : line.quantity_base ?? ''} disabled={busy} onChange={(event) => line.pricing_basis === 'PACKAGE' ? patchDraftLine(line.id, { packages_count: Number(event.target.value) }) : patchDraftLine(line.id, { quantity_base: event.target.value })} onBlur={(event) => line.pricing_basis === 'PACKAGE' ? saveLine(line, { packages_count: Number(event.target.value) }) : saveLine(line, { quantity_base: event.target.value })} /><span>{line.pricing_basis === 'PACKAGE' ? 'уп.' : line.unit_name_snapshot}</span></span>}</td>
        <td>{line.pricing_basis === 'FIXED_AMOUNT' ? '—' : <span className="input-with-suffix"><input aria-label={`Цена ${line.product_name_snapshot}`} type="number" min="0.01" step="0.01" value={line.pricing_basis === 'PACKAGE' ? line.price_per_package ?? '' : line.unit_price ?? ''} disabled={busy} onChange={(event) => line.pricing_basis === 'PACKAGE' ? patchDraftLine(line.id, { price_per_package: event.target.value }) : patchDraftLine(line.id, { unit_price: event.target.value })} onBlur={(event) => line.pricing_basis === 'PACKAGE' ? saveLine(line, { price_per_package: event.target.value }) : saveLine(line, { unit_price: event.target.value })} /><span>₽ / {line.pricing_basis === 'PACKAGE' ? 'уп.' : line.unit_name_snapshot}</span></span>}</td>
      ...
      <div className="allocation-summary"><div><span>Итого по документу</span><strong>{formatMoney(draft.total_amount)}</strong></div>{draft.supplier_confirmation_revision && <div><span>Источник defaults</span><strong>Ответ поставщика, ревизия {draft.supplier_confirmation_revision}</strong></div>}</div>
